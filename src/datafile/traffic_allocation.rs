...
impl TrafficAllocation {
    pub fn build(datafile: &mut JsonValue, variations: &mut HashMap<String, Rc<Variation>>) -> Result<TrafficAllocation> {
        ...
        let get_allocation = |value: &mut JsonValue| -> Result<(u32, Rc<Variation>)> {
            ...
            // NOTE: the datafile might contain the some variation multiple times in the traffic allocation
